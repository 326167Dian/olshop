...
@section('header', 'Ubah Operator')

@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Form Ubah Data Operator</h3>
        </div>

        <form action="{{ route('inventory.admin.update', $admin->id_admin) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-md-4 form-group">
                        ...
                        <small class="text-muted">Apabila password tidak diubah, dikosongkan saja.</small>
                        @error('password') <span class="invalid-feedback d-block">{{ $message }}</span> @enderror
                    </div>
                    <div class="col-md-4 form-group">
                        <label for="akses_level">Akses Level</label>
                        <select class="form-control" name="akses_level" id="akses_level">
                            <option value="pemilik" {{ old('akses_level', $admin->akses_level) == 'pemilik' ? 'selected' : '' }}>Pemilik</option>
                            <option value="petugas" {{ old('akses_level', $admin->akses_level) == 'petugas' ? 'selected' : '' }}>Petugas</option>
                        </select>
                    </div>
                </div>
                ...
                <div class="row">
                    ...
                    <div class="col-md-2 form-group">
                        ...
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="blokir" id="blokir_y" value="Y" {{ $admin->blokir == 'Y' ? 'checked' : '' }}>
                            <label class="form-check-label" for="blokir_y">Y</label>
                        </div>
                        <div class="form-check form-check-inline">
                            <input class="form-check-input" type="radio" name="blokir" id="blokir_n" value="N" {{ $admin->blokir == 'N' ? 'checked' : '' }}>
                            ...
                            @foreach ($items as $column => $label)
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="{{ $column }}"
                                        id="{{ $column }}" value="Y" {{ $admin->{$column} == 'Y' ? 'checked' : '' }}>
