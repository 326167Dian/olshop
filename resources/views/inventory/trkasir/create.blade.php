...
@section('header', 'Tambah Penjualan')

@section('content')
    <div class="card card-primary">
        <div class="card-header">
            <h3 class="card-title">Tambah Penjualan -- {{ $kdTransaksi }}</h3>
        </div>
        <div class="card-body">
            ...
            <div class="row">
                <div class="col-lg-6">
                    <div class="form-group">
                        <label>Tanggal</label>
                        <input type="date" class="form-control" id="tgl_trkasir" value="{{ now()->format('Y-m-d') }}">
                    </div>
                    <div class="form-group">
                        <label>Kasir</label>
                        <input type="text" class="form-control" value="{{ $admin->nama_lengkap }}" disabled>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="kodetx">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" class="form-control" id="ket_trkasir">
                    </div>

                    <div class="form-group" id="groupPoin" style="display:none">
                        <label>
                            <input type="checkbox" id="use_poin"> Gunakan Poin (tersedia: <span id="total_poin_member">0</span>)
                        </label>
                        <input type="number" class="form-control" id="redeem_poin" value="0" min="0" readonly>
                    </div>
                </div>
            ...
            <div class="mt-3">
                <button type="button" class="btn btn-primary" id="btnSimpanTransaksi">(F3) Simpan Transaksi</button>
                <a href="{{ route('inventory.trkasir.index') }}" class="btn btn-secondary">Batal</a>
            </div>
        </div>
    </div>

    <!-- Modal Item (barang + bundle) -- dilebarkan (mengikuti legacy 'width:90%') supaya
         kolom Indikasi cukup luas dibaca saat karyawan mencari obat berdasarkan indikasi. -->
...
<script>
    ...
    // --- Kalkulasi total (server tetap otoritatif saat submit, ini hanya pratinjau) ---
    ...
    document.getElementById('redeem_poin').addEventListener('input', recalcTotal);
    ...
    function fillItemForm(d) {
        ...
        // Barang berganti -> batch yang sebelumnya dipilih (kalau ada) tidak relevan lagi.
        document.getElementById('no_batch').value = '';
        document.getElementById('exp_date_display').value = '';
    }

    // --- Modal Batch (#ModalBatch, mengikuti batch_serverside.php) ---
    ...
    function tambahKeranjang() {
        ...
        // Petugas Pelayanan yang berlaku SAAT baris ini ditambahkan -- kalau belum
        // dipilih, server otomatis menjadikan kasir (id_admin) sebagai default.
    ...
    // --- Inline edit qty/resep & hapus baris (delegasi, sintas reload #tabeldata) ---
    ...
    $(document).on('click', '.btn-pilih-pelanggan', function() {
        ...
        document.getElementById('max_poin').value = d.total_poin || 0;
        document.getElementById('total_poin_member').textContent = d.total_poin || 0;
        document.getElementById('groupPoin').style.display = '';
        document.getElementById('use_poin').checked = false;
        document.getElementById('redeem_poin').value = 0;
        document.getElementById('redeem_poin').readOnly = true;
        recalcTotal();
    ...
    document.getElementById('btnHapusPelanggan').addEventListener('click', function() {
        ...
        document.getElementById('groupPoin').style.display = 'none';
        document.getElementById('redeem_poin').value = 0;
        recalcTotal();
    });
    document.getElementById('use_poin').addEventListener('change', function() {
        var redeemEl = document.getElementById('redeem_poin');
        redeemEl.readOnly = !this.checked;
        if (!this.checked) redeemEl.value = 0;
        recalcTotal();
    });
    document.getElementById('redeem_poin').addEventListener('input', function() {
        var max = parseAngka(document.getElementById('max_poin').value);
        if (parseAngka(this.value) > max) { alert('Poin melebihi yang dimiliki pelanggan.'); this.value = max; }
    });

    // --- Simpan Transaksi (finalisasi) ---
    function simpanTransaksi() {
        // Petugas Pelayanan boleh dikosongkan -- server otomatis menjadikan kasir
        // (operator yang sedang login) sebagai default kalau tidak dipilih.
        ...
        if (!document.querySelector('#tabelKeranjang tbody tr[data-id]')) { alert('Belum ada item dalam transaksi.'); return; }
        ...
        form.append('redeem_poin', parseAngka(document.getElementById('redeem_poin').value));

        fetch("{{ route('inventory.trkasir.store') }}", { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}', 'Accept': 'application/json' }, body: form })
            .then(function(res) { return res.json().then(function(d) { return { ok: res.ok, d: d }; }); })
            .then(function(r) {
                if (!r.ok) { alert(r.d.message || 'Proses gagal, periksa kembali data yang diisi.'); return; }
                window.open("{{ url('inventory/trkasir') }}/" + r.d.id_trkasir + "/struk", '_blank');
    ...
    document.addEventListener('keydown', function(event) {
        if (event.key === 'F1') { event.preventDefault(); tambahKeranjang(); }
