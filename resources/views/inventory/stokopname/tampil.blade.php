...
@section('content')
    ...
    <div class="card">
        ...
        <div class="card-body table-responsive">
            ...
            <center><strong>STOK OPNAME RAK {{ $jenisobat }}<br>Tanggal : {{ $tgl }}</strong></center>
            <hr>
...
<script>
    ...
    function simpanStokOpname(param) {
        ...
        var expDate = $('#exp_date_' + param).val();
        var jml = $('#jml_' + param).val();
        ...
        $.ajax({
            type: 'POST',
            url: "{{ route('inventory.stokopname.store') }}",
            data: {
                _token: "{{ csrf_token() }}",
                id_barang: idBarang,
                kd_barang: kdBarang,
    ...
    function hapusStokOpname(id) {
        ...
        $.ajax({
            type: 'POST',
            url: "{{ url('inventory/stokopname') }}/" + id,
            data: {
                _token: "{{ csrf_token() }}",
                _method: 'DELETE'
            },
    ...
    function tabelStokopname() {
        ...
        $.ajax({
            url: "{{ route('inventory.stokopname.grid') }}?" + $.param({ tgl: tglAwal, jenisobat: jenisobat }),
            type: 'GET',
    ...
    function tabelStokopnameRekap() {
        ...
        $.ajax({
            url: "{{ route('inventory.stokopname.rekap') }}?" + $.param({ tgl: tglAwal, jenisobat: jenisobat }),
            type: 'GET',
            success: function (data) {
