    ...
    <div class="card card-primary">
        ...
        <form action="{{ route('inventory.supplier.store') }}" method="POST">
            ...
            <div class="card-body">
                ...
                <div class="form-group">
                    <label for="tlp_supplier">Telp</label>
                    <input type="text" name="tlp_supplier" id="tlp_supplier" class="form-control"
                        value="{{ old('tlp_supplier') }}">
                </div>
                <div class="form-group">
                    <label for="alamat_supplier">Alamat</label>
                    <textarea name="alamat_supplier" id="alamat_supplier" class="form-control" rows="3">{{ old('alamat_supplier') }}</textarea>
                </div>
                <div class="form-group">
                    <label for="ket_supplier">Keterangan</label>
                    <textarea name="ket_supplier" id="ket_supplier" class="form-control" rows="3">{{ old('ket_supplier') }}</textarea>
                </div>
            </div>
